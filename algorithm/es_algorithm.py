...
class ESAlgorithm(Algorithm):
    def __init__(self, episode, model_type, model_params, iter_per_agent , mutation_lr, min_mutation_step, initial_mutation_step, logger_filename):
    ...
    def get_mutation_step(self, parents):
        """ Compute mutation step size for new child based on properties of its parents """
        # Compute new mutation step size
        parent_rewards = [parent.total_reward for parent in parents]
        reward_fraction = self.iter_per_agent * 10 / np.average(parent_rewards) 
        ...
        # If new mutation step size is below the minimum, set it to the minimum
